...
const ProductCard = ({ product }) => {
  ...
  const thumbnail2 = env.app_url + product?.thumbnails?.[0]?.filepath;
  const alt1 = product?.thumbnails[0]?.alternateText;
  const alt2 = product?.thumbnails[1]?.alternateText;
    ...
    <>
      ...
      >
        ...
        >
          ...
          {product?.teaFormat?.assortment ? (
            <p className="text-brand__font__size__xs font-brand__font__500 capitalize flex items-center gap-1">
              {product?.teaFormat?.thumbnail?.length && (
                <img
                  className="w-[12px] h-[12px]"
                  src={env.app_url + product?.teaFormat?.thumbnail[0].filepath}
                  alt={product?.teaFormat?.thumbnail[0]?.alternateText || ""}
                />
              )}
              <span> {product?.teaFormat?.assortment}</span>
            </p>
          ) : (
